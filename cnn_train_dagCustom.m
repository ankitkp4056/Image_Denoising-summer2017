function [net,stats] = cnn_train_dagCustom(net, imdb, getBatch, varargin)
...
%%% GradientClipping
opts.gradClipping = true;
opts.theta_gradClipping = 0.005;
...
if exist('denoise.mat', 'file') == 2
netStruct = load('denoise.mat') ;
net = dagnn.DagNN.loadobj(netStruct) ;
clear netStruct ;
end 

%Loading saved net /Initializing new state (if opts.continue == false)
...
% Saving the net after training
netStruct = net.saveobj() ;
save('denoise.mat', '-struct', 'netStruct') ;
clear netStruct ;
...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
	  ...
	  % Implementing GradientClipping
	  if params.gradClipping
		theta = params.theta_gradClipping / thisLR;
		net.params(p).value = gradClipping(net.params(p).value, theta);  
      end 
...
%%%-------------------------------------------------------------------------
function A = gradClipping(A, theta)
    A(A>theta)  = theta;
    A(A<-theta) = -theta;
